...
// Convierte la hora (America/Lima, UTC-5) a hora local del servidor
function toLocalTime(diaryHour) {
  if (!diaryHour) return ''
  const [hh, mm] = diaryHour.split(':').map(Number)
  if (Number.isNaN(hh) || Number.isNaN(mm)) return diaryHour.slice(0, 5)
  const d = new Date()
  const limaUtc = Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), hh + 5, mm)
  const local = new Date(limaUtc)
...
function parseAgendaJson(json) {
  ...
  for (const item of arr) {
    ...
    events.push({
      title,
      time: toLocalTime(attr.diary_hour),
      sport: sport.name,
      emoji: sport.emoji,
