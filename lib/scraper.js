...
const { getDeporflixEvents } = require('./scraper-deporflix')
...
// Fuente complementaria (p. ej. deporflix): NO crea eventos propios. Solo agrega
// sus streams a los eventos ya existentes (de las fuentes principales) cuyo
// partido matchee — por titulo normalizado o, como fallback, por pares de equipos.
// Si un evento complementario no matchea ningun existente, se descarta.
function mergeOnlyStreams(existing, extras) {
  const { eventTeamKeys } = require('./footballApi')
  const includesOrContains = (a, b) => a.includes(b) || b.includes(a)
  ...
  for (const e of extras || []) {
    ...
    if (!target) {
      const k1 = eventTeamKeys(e.title)
      if (k1.length >= 2) {
        for (const cand of existing) {
          const k2 = eventTeamKeys(cand.title)
          if (k2.length >= 2) {
            const hit =
              (includesOrContains(k2[0], k1[0]) && includesOrContains(k2[1], k1[1])) ||
              (includesOrContains(k2[0], k1[1]) && includesOrContains(k2[1], k1[0]))
            if (hit) {
              target = cand
              break
            }
    ...
    if (target) {
      target.streams = mergeStreams(target.streams, e.streams)
      if (!target.flagUrl && e.flagUrl) target.flagUrl = e.flagUrl
      if (!target.country && e.country) target.country = e.country
    }
  }
...
async function getEvents() {
  ...
  // Las 3 fuentes principales en paralelo; si una falla, no tumba al resto.
  // deporflix va aparte: solo enriquece eventos ya creados (mergeOnlyStreams).
  const [alangulotv, futbollibre, agenda18, deporflix] = await Promise.allSettled([
  ...
  mergeOnlyStreams(merged, deporflix.status === 'fulfilled' ? deporflix.value : [])
